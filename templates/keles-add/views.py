...
def InvoiceCreate(request):
    if request.method == 'POST':
        ...
        for name_id, size_id, color_id, product_to_id, quantity in zip(name_ids, size_ids, color_ids, product_to_ids, quantities):
            name = CategoryModel.objects.get(id=name_id)
            size = SizeModel.objects.get(id=size_id)
            color = ColorModel.objects.get(id=color_id)
        ...
        # Save created IDs to session
        request.session['last_invoice_ids'] = created_ids

        # Redirect to export, which will redirect to invoice list after download
        return redirect('add:export_to_excel')  # URL must be defined
...
def AddRemaing(request):
    if request.method == 'POST':
        ...
        return redirect('keles:remaing_list')  # change this to your actual url name
...
def EntryCreate(request):
    if request.method == 'POST':
        ...
        return redirect('keles:product_in-list')  # Change this to your actual URL name
...
def InvoiceEdit(request, pk):
    obj = get_object_or_404(InvoiceCreateKeles, pk=pk)
    if request.method == 'POST':
        form = InvoiceCreateForm(request.POST, instance=obj)
        if form.is_valid():
            form.save()
            return redirect('keles:invoice-list')  # replace with your actual detail or success view
    else:
        form = InvoiceCreateForm(instance=obj)
    return render(request, 'edit/invoice-edit.html', {'form': form})
...
def InvoiceDelete(request, pk):
    query = get_object_or_404(InvoiceCreateKeles, pk=pk)
    if request:
        query.delete()
        return redirect('keles:invoice-list')  # Change to your actual URL name
